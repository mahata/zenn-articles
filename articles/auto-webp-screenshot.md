---
title: "macOSのスクリーンショットを自動的にWebP形式に変換する方法"
emoji: "💽"
...
macOSのデフォルト機能でスクリーンショットを撮ると `.png` 形式で保存されます。しかし、Web用途では `.webp` 形式の方がファイルサイズが小さくて便利です。この記事では、macOSでスクリーンショットを撮ると自動的に `.webp` 形式に変換されるようにする方法を紹介します。
